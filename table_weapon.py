...
from table_skill import dump_skill_common_data, dump_skill_data
    ...
    for i in range(len(skill_names)):
        ...
        for j in range(len(skill_names[i])):
            ...
            if level == 0:
                ...
                row[f"Skill{j}"] = f"{skill_names[i][j]}: {level}"
        sub_table.append(row)
    sub_table = pd.DataFrame(sub_table)
    df = pd.concat([df.drop(columns=["Skill", "SkillLevel"]), sub_table], axis=1)

    df = reindex_column(
        df, ["Skill0", "Skill1", "Skill2", "Skill3"], next_to="SlotLevel"
    )
